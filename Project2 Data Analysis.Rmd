...
#bankfull$pdays <- ifelse(bankfull$pdays == 999, NA, bankfull$pdays)
...
mutate(bankfull, prev = as.factor(previous)) %>% ggplot(aes(prev, y, fill=y)) + geom_col()
...
```{r LDA analysis with bankcont, cache=TRUE}
...
```{r LDA analysis with bankbin, cache=TRUE}
...
```{r PCA Analysis with bankCont, cache=TRUE}
...
```{r PCA Analysis with bankbin, cache=TRUE}
...
# Build model:
mylogit <- glm(y ~ age+job+marital+education+default+housing+loan+contact+month+day_of_week+duration+campaign+pdays+previous+poutcome+emp.var.rate+cons.price.idx+cons.conf.idx+euribor3m+nr.employed
                 ,data = bankfull, family = "binomial")

# Summary of everything model:
summary(mylogit)

# Recommend we drop: marital, age, loan, previous, housing, education
mylogit2 <- glm(y ~ job+default+contact+month+day_of_week+duration+campaign+pdays+poutcome+emp.var.rate+cons.price.idx+cons.conf.idx+euribor3m+nr.employed
                 ,data = bankfull, family = "binomial")
summary(mylogit2)

# SP Recommend we drop: duration, pdays, nr.employed, default, euroibor3
mylogit3 <- glm(y ~ job+default+contact+month+campaign+poutcome+emp.var.rate+cons.price.idx+cons.conf.idx
                 ,data = bankfull, family = "binomial")
summary(mylogit3)
```
