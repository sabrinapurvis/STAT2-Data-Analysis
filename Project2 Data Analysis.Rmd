...
### PCA
Three PCA plots are shown below that display separation between the most highly correlated features found in the data. 

##### ![](./figures/PCA.png){width=10%} 
...
pred.DT = predict(classifier, newdata = testdata[-18], type = 'prob')

# find the threshold for prediction optimization
predictions_DT <- data.frame(y = testdata$y, pred = NA)
...
test.eval.DT = binclass_eval(as.integer(testdata[,18])-1, pred.DT[,2] > 0.36)
...
# calculate ROC curve
rocr.pred = prediction(predictions = pred.DT[,2], labels = testdata$y)
...
```{r GLM Logistic, cache=TRUE, eval=FALSE}
