...
![](./figures/probslogistic.png){width="7.5in"}
...
g1 <- percentagePlot(df, fct_rev(df$job), "Job Type") + coord_flip()
g2 <- percentagePlot(df, fct_rev(df$education), "Education Level") + coord_flip() 
g3 <- percentagePlot(df, df$contact, "Contact Method") 
g4 <- percentagePlot(df, fct_rev(df$month), "Month") + coord_flip()
g5 <- percentagePlot(df, fct_rev(df$day_of_week), "Day of the Week") + coord_flip()
ggarrange(g1, g2, g3, g4, g5, ncol = 3, nrow=2)

# Do we want to show the non-selected feature plots?
g6 <- percentagePlot(df, df$marital, "Marital Status") 
g7 <- percentagePlot(df, df$default, "Default") 
g8 <- percentagePlot(df, df$housing, "Housing") 
g9 <- percentagePlot(df, df$loan, "Loan") 
g10 <- percentagePlot(df, df$previous, "Previous") + coord_flip()
g11 <- percentagePlot(df, df$poutcome, "Poutcome") 
ggarrange(g6, g7, g8, g9, g10, g11, ncol = 3, nrow=2)

g12 <- df %>% ggplot(aes(month, y,  fill=y)) + geom_col()
g13 <- df %>% ggplot(aes(day_of_week, y,  fill=y)) + geom_col()

g14 <- df %>% ggplot(aes(campaign, y,  fill=y)) + geom_col()
g15 <- df %>% ggplot(aes(y, campaign)) + geom_boxplot() + coord_flip()

g16 <- df %>% ggplot(aes(pdays, y,  fill=y)) + geom_boxplot()

g17 <- df %>% ggplot(aes(previous, y,  fill=y)) + geom_col()

g18 <- df %>% ggplot(aes(emp.var.rate, y,  fill=y)) + geom_col()
g19 <- df %>% ggplot(aes(y, emp.var.rate)) + geom_boxplot() + coord_flip()

g20 <- df %>% ggplot(aes(cons.price.idx, y,  fill=y)) + geom_col()
g21 <- df %>% ggplot(aes(y, cons.price.idx)) + geom_boxplot() + coord_flip()

g22 <- df %>% ggplot(aes(cons.conf.idx, y,  fill=y)) + geom_col()
g23 <- df %>% ggplot(aes(y, cons.conf.idx)) + geom_boxplot() + coord_flip()


g24 <- df %>% ggplot(aes(euribor3m, y,  fill=y)) + geom_col()
g25 <- df %>% ggplot(aes(y, euribor3m)) + geom_boxplot() + coord_flip()

g26 <- df %>% ggplot(aes(nr.employed, y,  fill=y)) + geom_col()
g27 <- df %>% ggplot(aes(y, nr.employed)) + geom_boxplot() + coord_flip()
ggarrange (g12, g13, g14, g15, g16, g17, g18, g19, g20 ,g21, g22, g23, g24, g25, g26, g27, ncol = 4, nrow = 4)
