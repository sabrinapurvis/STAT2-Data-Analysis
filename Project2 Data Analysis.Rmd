...
![](./figures/LRT.png){width="4.5in"} 

We also can perform the  Wald test (Anova(simple.model, type="II", test="Wald")) to get the similar result as Likelihood Ratio Test.  Although this analysis does not require the dependent and independent variables to be related linearly, it requires that the independent variables are linearly related to the log odds. By  p values < 0.05,  we reject the null hypothesis. It means that at least one explanatory variable in the model is significant.   
...
>>>>>>> d35bbcce9f403ef3196111b5ba6dad031973cc51
...
During modeling of categorical KNN, we utilized a full model, with all variables in it from the simple logistic regression model and performed ten iterations of cross validation.  Using KNN with scaled, continuous predictors and the same variables broken out in a one-hot encoded fashion, we were able to obtain much better _______ (sensitivity?) metrics.  We employed a hyper-tuning approach to find the best value of K, retraining the model on each value of K up to the `sqrt` of the number of observations in our model.  It was determined the best value of K was 60, and this is what was used for the tuned KNN model.  
