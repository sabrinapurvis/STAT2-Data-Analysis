...
We also can perform the  Wald test (Anova(simple.model, type="II", test="Wald")) to get the similar result as Likelihood Ratio Test.  Although this analysis does not require the dependent and independent variables to be related linearly, it requires that the independent variables are linearly related to the log odds. 
...
We evaluated the Cook's D Plot and Leverage plot to assess assumption violations.  Cook's D did not return a value above 0.0020, showing that there are no outliers to consider.  Leverage also supports this, with only one value above 0.020.  
  ![](./figures/ResidualLEverage.png){width="4.5in"}
